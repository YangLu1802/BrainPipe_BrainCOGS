...
# run elastix
# OUT3=$(sbatch --parsable --dependency=afterok:${OUT2##* } --export=ALL,output_directory=${output_directory} --array=0-${max_array_job_step2and3} slurm_files/step3.sh) 
# echo $OUT3
OUT3=$(sbatch --parsable --export=ALL,output_directory=${output_directory} --array=0-${max_array_job_step2and3} slurm_files/step3.sh) 
echo $OUT3
